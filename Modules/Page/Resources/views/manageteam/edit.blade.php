      ...
      <div class="row">
        <div class="col-md-6 col-12">
          <div class="card">
            ...
            <div class="card-body">
              <form class="form form-horizontal" action="{{ url('/management-team/'.$test->id) }}" method="post" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="row">
                  ...
                  <div class="col-12">
                    @php
                      $existingImg = $test->images
                        ? asset('backend/teams/'.$test->images)
                    ...
                    <div class="form-group row align-items-start">
                      ...
                      <div class="col-sm-9">
                        <div class="tm-uploader">
                          <label for="account-upload" class="pick" id="tmPick" role="button" aria-label="Choose image"
                  ...
                  --}}
                  <input type="hidden" name="alt_tag" value="{{ $test->alt_tag }}">

                  <div class="col-12">
                    <div class="form-group row">
                      <div class="col-sm-3 col-form-label"><label for="name">Name</label></div>
                      <div class="col-sm-9"><input type="text" id="name" class="form-control" name="name" value="{{$test->name}}" placeholder="" /></div>
                    </div>
                  </div>
                  ...
                  <div class="col-12">
                    <div class="form-group row">
                      ...
                      <div class="col-sm-9"><textarea name="description" class="form-control" rows="10">{{$test->description}}</textarea></div>
                    </div>
                  </div>
                  --}}
                  <textarea name="description" hidden>{{ $test->description }}</textarea>
                  ...
                  <div class="col-12">
                    <div class="form-group row">
                      ...
                      <div class="col-sm-9"><input type="text" id="rating" class="form-control" name="rating" value="{{$test->rating}}" /></div>
                    </div>
                  </div>
                  --}}
                  <input type="hidden" name="rating" value="{{ $test->rating }}">
                  ...
                  --}}
                  <input type="hidden" name="page_title" value="{{ $test->page_title }}">
                  ...
                  --}}
                  <input type="hidden" name="page_keywords" value="{{ $test->page_keywords }}">
...
(function(){
  ...
  function setPreview(url){
    if (!url){
      // show original (if any) or empty placeholder style
      if (existing){ pick.style.backgroundImage = `url('${existing}')`; pick.classList.remove('placeholder'); }
  ...
  // Bind
  document.getElementById('tmPick')?.addEventListener('click', (ev)=>{ ev.preventDefault(); openDialog(); });
  document.getElementById('tmChangeBtn')?.addEventListener('click', (ev)=>{ ev.preventDefault(); openDialog(); });
  document.getElementById('tmClearBtn')?.addEventListener('click', (ev)=>{
    ev.preventDefault();
    fileInput.value = '';
