...
<div class="content-wrapper">
  ...
  <div class="content-body">
    <div class="card event-form-card">
      <div class="card-body">
        <form class="mt-2" id="event-edit-form" action="{{ route('admin.events.update', $id ?? optional($event)->id) }}" method="POST" enctype="multipart/form-data">
          @csrf
...
<script>
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    // Sync both on submit and on clicking submit button
  ...
  // Quill image upload handler (mirror working page/post editor)
  window.addEventListener('load', function () {
    ...
    toolbar.addHandler('image', function () {
      ...
      input.onchange = function () {
        ...
        fetch('/upload-quill-image', { method:'POST', body:fd, headers:{ 'X-CSRF-TOKEN': token }, credentials:'same-origin' })
          .then(res => res.text().then(text => { if(!res.ok) throw new Error('Upload failed '+res.status); return JSON.parse(text); }))
