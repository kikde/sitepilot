...
@section('content')
@php
  $all_categories = $all_categories ?? collect();
...
<script>
  ...
  // Quill image upload handler (same as page editor)
  window.addEventListener('load', function () {
    if (!window.Quill) return;
    const editorEl = document.querySelector('#blog-editor-container .editor');
    if (!editorEl) return;
    let quill = null;
    try { quill = Quill.find(editorEl) || editorEl.__quill || null; } catch (e) { quill = editorEl.__quill || null; }
    if (!quill) return;
    ...
    toolbar.addHandler('image', function () {
      ...
      input.onchange = function(){ const f=input.files&&input.files[0]; if(!f) return; const fd=new FormData(); fd.append('image',f);
        const tokenMeta = document.querySelector('meta[name="csrf-token"]');
        const token = tokenMeta ? tokenMeta.getAttribute('content') : '';
        fetch('/upload-quill-image',{method:'POST',body:fd,headers:{'X-CSRF-TOKEN':token},credentials:'same-origin'})
          .then(res=>res.text().then(t=>{ if(!res.ok) throw new Error('Upload '+res.status); return JSON.parse(t); }))
          .then(data=>{ if(data&&data.url){ const rng=quill.getSelection(true)||{index:quill.getLength()}; quill.insertEmbed(rng.index,'image',data.url,'user'); quill.setSelection(rng.index+1);} })
          .catch(()=>{});
      };
      input.click();
    });
    ...
    const form = document.getElementById('event-form');
    form?.addEventListener('submit', function(){ const hidden=document.getElementById('description'); if(hidden) hidden.value = quill.root.innerHTML; });
    // Keep hidden synced on Quill edits, too
    quill.on('text-change', function(){ const hidden=document.getElementById('description'); if(hidden) hidden.value = quill.root.innerHTML; });
  });
