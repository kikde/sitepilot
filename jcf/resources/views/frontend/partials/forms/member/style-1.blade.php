...
/* Buttons */
.btn{ appearance:none; -webkit-appearance:none; border:1px solid transparent; cursor:pointer; padding:10px 14px; border-radius:10px; font-weight:600; font-size:14px; }
.btn-primary{ background:linear-gradient(135deg,var(--brand),var(--brand2)); color:#fff; box-shadow:0 8px 20px rgba(255,77,77,.25); -webkit-text-fill-color:#fff; -webkit-tap-highlight-color:rgba(0,0,0,0); }
.btn-primary:hover{ transform:translateY(-1px); box-shadow:0 10px 26px rgba(255,77,77,.35); }
/* Fix: some mobile browsers temporarily drop gradient/text color on tap */
.btn-primary:active,
.btn-primary:focus{
...
/* Keep gradient while submitting (class) or aria-busy set */
...
<script>
// Keep gradient after submit; prevent double submit
document.addEventListener('DOMContentLoaded', function(){
  var form = document.querySelector('.auth-register-form');
  if (!form) return;
  ...
  form.addEventListener('submit', function(){
    ...
    btn.disabled = true; // style remains gradient via CSS
  });
});
</script>
