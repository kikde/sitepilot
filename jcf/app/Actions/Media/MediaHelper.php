...
class MediaHelper
{
    ...
    public static function fetch_media_image($request,$type='admin')
    {
        ...
        if (!empty($selected_image)){
            if (file_exists(public_path('/backend/media-uploader/'.$selected_image->path))) {
        ...
        foreach ($all_images as $image){
            if (file_exists(public_path('/backend/media-uploader/'.$image->path))){
                $image_url = asset('/backend/media-uploader/'.$image->path);
                if (file_exists('/backend/media-uploader/grid-' . $image->path)) {
                    $image_url = asset('/backend/media-uploader/grid-' . $image->path);
                }
                $all_image_files[] = [
                    'image_id' => $image->id,
                    'title' => $image->title,
    ...
    public static function delete_media_image($request,$type ='admin')
    {
        ...
        if (file_exists('assets/uploads/media-uploader/' . $get_image_details->path)) {
            unlink('/backend/media-uploader/' . $get_image_details->path);
        }
        if (file_exists('/backend/media-uploader/' . $get_image_details->path)) {
            unlink('/backend/media-uploader/grid-' . $get_image_details->path);
        }
        if (file_exists('/backend/media-uploader/large-' . $get_image_details->path)) {
            unlink('/backend/media-uploader/large-' . $get_image_details->path);
        }
        if (file_exists('/backend/media-uploader/semi-large-' . $get_image_details->path)) {
            unlink('/backend/media-uploader/semi-large-' . $get_image_details->path);
        }
        if (file_exists('/backend/media-uploader/thumb-' . $get_image_details->path)) {
            unlink('/backend/media-uploader/thumb-' . $get_image_details->path);
        }
    ...
    public static function insert_media_image($request,$type='admin'){
        ...
        if ($request->hasFile('file')) {
            ...
            $folder_path = public_path('/backend/media-uploader');
    ...
    public static function load_more_images($request,$type = 'admin'){
        ...
        if ($type === 'web'){
            $image_query->where(['type' => $type,'user_id' => A($type)->id()]);
        }
