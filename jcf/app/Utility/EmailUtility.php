...
class EmailUtility
{
    ...
    //    $mailid = 'kikde.sara@gmail.com';
     
        $user           = Product::where('id',$user_id)->first();
        ...
        $email_body     = str_replace('[[sitename]]', get_setting('site_url'), $email_body);
        ...
        try{
            Notification::route('mail', $user->email)->notify(new EmailNotification($subject, $email_body));
        //    Notification::send($mailid, new EmailNotification($email_body));
            // Mail::to(getenv('MAIL_TO'))->send(new UserMail($mailform));
        }
        catch(\Exception $e){
            dd($e);
        }
