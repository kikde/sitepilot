...
/** Show Razorpay checkout for last registered member */
 ...
 /**
    ...
    public function verify(\Modules\User\Entities\Payment $payment, \Illuminate\Http\Request $request)
    {
        $payment->is_verified = true;
        $payment->verified_at = now();
        $payment->save();

        // Activate user after verify
        $payment->user()->update([
            'useractive' => 1,
        ]);

        // Queue receipt render so list shows a downloadable receipt soon
        try {
            $userId = (int) $payment->user_id;
            if ($userId > 0) {
                $targetUrl = url('/payment-receipt/'.$userId);
                \Modules\User\Services\ApiShot::capture($userId, $targetUrl, [
                    'ext'             => 'pdf',
                    'pageSize'        => 'A4',
            ...
            \Log::warning('PAYMENT.VERIFY_RECEIPT_QUEUE_FAIL', ['user_id' => $payment->user_id, 'error' => $e->getMessage()]);
        }

        return back()->with('message', 'Payment verified and member activated. Receipt will attach shortly.');
    }

    /**
     * ADMIN: Delete a payment row (and any stored files)
     */
    public function destroy(\Modules\User\Entities\Payment $payment)
    {
        try {
            $paths = [];
            if (!empty($payment->screenshot))  { $paths[] = ltrim($payment->screenshot, '/'); }
            if (!empty($payment->payment_rec)) { $paths[] = ltrim($payment->payment_rec, '/'); }
            ...
            \Log::warning('PAYMENT.DELETE_FAIL', ['id' => $payment->id, 'error' => $e->getMessage()]);
