        ...
        <div class="col-12">
          ...
          <div class="breadcrumb-wrapper">
            <ol class="breadcrumb">
              ...
              <li class="breadcrumb-item"><a href="#">Create new page</a></li>
            </ol>
          </div>
        </div>
      </div>
      ...
      <div class="row">
        <div class="col-12">
          <div class="card">
            ...
            <div class="card-body">
              <!-- Form -->
              <form class="mt-2" action="{{url('/pages/'.$sectorlist->id)}}" method="post" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                  ...
                  <div class="col-md-6 col-12">
                    <div class="form-group mb-2">
                      ...
                      <input type="text" id="blog-edit-title" class="form-control"
                             name="sector_name" value="{{$sectorlist->sector_name}}"
                             onchange="fillslug(this.value)" placeholder="" />
                    </div>
                  </div>
                    ...
                    <div class="form-group mb-2">
                      ...
                      <div id="blog-editor-wrapper">
                        ...
                        <div id="blog-editor-container">
                          <div class="editor">{!! $sectorlist->description !!}</div>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                {{-- =======================
                     Beautiful Breadcrumb Image Picker (with existing preview)
                     ======================= --}}
                <div class="col-12">
                  @php
                    $existingSrc = !empty($sectorlist->breadcrumb)
                      ? asset('backend/uploads/'.$sectorlist->breadcrumb)
                      : '';
                  @endphp

                  <div class="border rounded p-2 breadcrumb-uploader">
                    <h4 class="mb-1">Upload Image</h4>

                    <label for="breadcrumbInput" class="pick {{ $existingSrc ? '' : 'placeholder' }}" id="bcPick"
                           role="button" aria-label="Choose breadcrumb image"
                    ...
                    <div class="hint">Ideal size: 700×300 (PNG/JPG). Keep file size reasonable for faster loads.</div>
                  </div>
                </div>
                {{-- ======================= /picker ======================= --}}
...
<script>
  ...
  (function(){
    ...
    const existing  = {!! json_encode($existingSrc ?? '') !!};
    let lastURL = null;
    ...
    // Clear = revert to existing image if any; otherwise placeholder
    clearBtn?.addEventListener('click', (ev)=>{
      ev.preventDefault();
      fileInput.value = '';
      if (lastURL){ URL.revokeObjectURL(lastURL); lastURL=null; }
      if (existing) setPreview(existing); else setPreview('');
    });

    // Reset form restores original state
